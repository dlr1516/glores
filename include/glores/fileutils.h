...
#define GLORES_PRINT_VARIABLE(X) std::cout << __FILE__ << "," << __LINE__ << ": " << #X << " " << (X) << std::endl; 

#define GLORES_PRINT_MSG(MSG) std::cout << __FILE__ << "," << __LINE__ << ": " << MSG << std::endl;

#define GLORES_PRINT_ERROR(MSG) std::cerr << __FILE__ << "," << __LINE__ << ": " << MSG << std::endl;

#define GLORES_ASSERT(COND) if (!(COND)) { std::cerr << __FILE__ << "," << __LINE__ << ": assertion failed on " << #COND << std::endl; exit(-1); }
...
namespace glores {
    ...
    /**
     * Saves a vector of points into a two-columns text file.
     * @param out output stream
     * @param varname variable name (not used at the moment!)
     * @param points the point vector to be saved
     * @param delim the delimiter
     */
    void savePointMatrix(std::ostream& out, std::string varname, const VectorPoint2d& points, std::string delim = ",");

    /**
     * Saves a vector of points into a two-columns text file stream.
     * First column contains coordinates x whereas the second one coordinates y.
     * @param out output stream
     * @param varname variable name (not used at the moment!)
     * @param points the point vector to be saved
     * @param delim the delimiter
    ...
    /**
     * Loads from a two-columns text input stream a vector of points. 
     * First column contains coordinates x whereas the second one coordinates y.
     * @param in the input stream
     * @param points the points loaded from stream
     * @param delim the delimiter
     */
